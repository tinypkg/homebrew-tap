...
### WinMux 自动更新
- **触发方式**: 每12小时定时检查 + 手动触发（可自动检测最新版本）
- **工作流**: `.github/workflows/update-winmux-version.yml`
- **功能**: 自动从 GitHub Releases 检测最新版本，下载 macOS ZIP，计算 SHA256 校验和
- **使用方法**:
  ```bash
  # 手动指定版本
  gh workflow run update-winmux-version.yml -f version=0.1.2

  # 自动检测最新版本
  gh workflow run update-winmux-version.yml
  ```

### ShichiZip 自动更新
- **触发方式**: 每12小时定时检查 + 手动触发（可自动检测最新版本）
- **工作流**: `.github/workflows/update-shichizip-version.yml`
- **功能**: 自动从 GitHub Releases 检测最新版本，下载 macOS arm64/x64 ZIP，计算 SHA256 校验和，同时更新 ShichiZip 和 ShichiZipZS
- **使用方法**:
  ```bash
  # 手动指定版本
  gh workflow run update-shichizip-version.yml -f version=0.0.3
...
### Herdr 自动更新
- **触发方式**: 每12小时定时检查 + 手动触发（可自动检测最新版本）
- **工作流**: `.github/workflows/update-herdr-version.yml`
- **功能**: 自动从 GitHub Releases 检测最新版本，下载 macOS arm64/x64 二进制文件，计算 SHA256 校验和
- **使用方法**:
  ```bash
  # 手动指定版本
...
### Con 自动更新
- **触发方式**: 每12小时定时检查 + 手动触发（可自动检测最新版本）
- **工作流**: `.github/workflows/update-con-version.yml`
- **功能**: 自动从 GitHub Releases 检测最新版本，下载 macOS arm64/x64 DMG，计算 SHA256 校验和，自动更新 URL 中的版本通道前缀（Beta/Dev/stable）
- **使用方法**:
  ```bash
  # 手动指定版本
  gh workflow run update-con-version.yml -f version=0.1.0-beta.60

  # 自动检测最新版本
  gh workflow run update-con-version.yml
  ```
...
### HorseMD 自动更新
- **触发方式**: 每周一 10:00 UTC 定时检查 + 手动触发（可自动检测最新版本）
- **工作流**: `.github/workflows/update-horseMD-version.yml`
- **功能**: 自动从 GitHub Releases 检测最新版本，下载 macOS ARM64 DMG，计算 SHA256 校验和
- **使用方法**:
  ```bash
  # 手动指定版本
  gh workflow run update-horseMD-version.yml -f version=0.1.5
